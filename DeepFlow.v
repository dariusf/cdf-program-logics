...
From CDF Require Import Common Sequences Tactics Heaps HeapTactics.
(* From CDF Require Import Separation2. *)
From CDF Require Import DeepSeparation.
...
(* Definition pure := pure val. *)

Definition empty := ens None (pi (ptrue val)).

Infix ";;" := seq (at level 80, right associativity).

(* Fixpoint replace_val x v p := *)

Fixpoint replace_ret (x:ident) (f:flow) :=
  match f with
  | ens (Some r) q => ens None (replace_sl r x q)
  | ens None _ => f
  | req _ => f
  | a ;; b => a ;; (replace_ret x b)
...
Proof.
  (* Check flow_mut. *)
  apply (flow_mut (fun (e:expr) => True)
    (fun (v:val) => True)
    (fun (f0:flow) => forall (x i:ident),
      fresh_in_flow i (replace_ret x f0) ->
      fresh_in_flow i f0)); auto.
  -
  (* ens case, where there is now a binder *)
  intros. destruct o; auto. unfold not; intros; subst. simpl in H. admit.
